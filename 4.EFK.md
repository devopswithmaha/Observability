...
# 🏠 Architecture
![Project Architecture](images2/architecture.gif)
...
```bash
eksctl create iamserviceaccount --name ebs-csi-controller-sa --namespace kube-system --cluster observability1 --role-name AmazonEKS_EBS_CSI_DriverRole --role-only --attach-policy-arn arn:aws:iam::aws:policy/service-role/AmazonEBSCSIDriverPolicy --approve
```
...
```bash
eksctl create addon --cluster observability1 --name aws-ebs-csi-driver --version latest  --service-account-role-arn  arn:aws:iam::569085067580:role/AmazonEKS_EBS_CSI_DriverRole --force
```
...
> **Windows / PowerShell** — `base64 -d` is not available, so decode with .NET instead:
```powershell
# for username
[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String((kubectl get secrets --namespace=logging elasticsearch-master-credentials -o jsonpath='{.data.username}')))
# for password
[System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String((kubectl get secrets --namespace=logging elasticsearch-master-credentials -o jsonpath='{.data.password}')))
```
