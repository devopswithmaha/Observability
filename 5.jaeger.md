...
# 🏠 Architecture
![Project Architecture](images333/architecture.gif)
...
- We have already instrumented our code using OpenTelemetry libraries/packages. For more details, refer to `application/myapp-a/tracing.js` or `application/myapp-b/tracing.js`.
...
```powershell
kubectl get secret elasticsearch-master-certs -n logging -o jsonpath='{.data.ca\.crt}' | ForEach-Object { [System.Convert]::FromBase64String($_) } | Set-Content -Path ca-cert.pem -Encoding Byte
```
...
kubectl delete configmao jeager-tls -n tracing
kubectl delete secrets es-tls-secret  -n tracing
kubectl delete secrets jeager-elasticsearch  -n tracing
